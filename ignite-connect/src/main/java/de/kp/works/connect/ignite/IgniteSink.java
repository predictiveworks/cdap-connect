...
public class IgniteSink extends BatchSink<StructuredRecord, IgniteCacheWritable, NullWritable> {
	...
	@Override
	public void transform(StructuredRecord input, Emitter<KeyValue<IgniteCacheWritable, NullWritable>> emitter) {
		emitter.emit(new KeyValue<>(new IgniteCacheWritable(input), null));
	}
