...
public class SAPHanaSink extends JdbcSink<SAPHanaWritable> {
	...
	private final SAPHanaSinkConfig config;
	private final SAPHanaConnect connect;
	...
	@Override
	public void transform(StructuredRecord input, Emitter<KeyValue<NullWritable, SAPHanaWritable>> emitter) {
		emitter.emit(new KeyValue<>(null, new SAPHanaWritable(connect, input)));
	}
