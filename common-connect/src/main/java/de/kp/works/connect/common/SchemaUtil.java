...
public class SchemaUtil implements Serializable {
	...
	public static Schema inferSchema(List<JsonObject> jsonArray, Integer samples) {
		...
		return Schema.recordOf("jsonSchema", fields);
	...
	public static Schema.Field inferField(List<Schema> schemas, String fieldName) {
		...
		if (isNullable && !fieldSchema.isNullable())
	...
	private static Integer getIndex(JsonArray jsonArray) {
		...
		int count = Math.min(10, jsonArray.size());
	...
	public static Boolean isStringType(Schema.Type fieldType) {
		return fieldType.equals(Schema.Type.STRING);
	}

	public static Boolean isTimeType(Schema.Type dataType) {
		return dataType == Schema.Type.LONG;
	}

	public static Boolean isIntegerType(Schema.Type dataType) {
		return dataType == Schema.Type.INT;
	}
