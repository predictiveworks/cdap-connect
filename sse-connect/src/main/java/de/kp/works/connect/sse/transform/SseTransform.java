package de.kp.works.connect.sse.transform;
/*
...
import de.kp.works.connect.sse.SseConfig;
import de.kp.works.connect.sse.SseUtil;
...
public class SseTransform implements Function<JavaRDD<String>, JavaRDD<StructuredRecord>> {

	private Schema schema;

	public SseTransform(SseConfig config) {
	}
	...
	public JavaRDD<StructuredRecord> call(JavaRDD<String> input) throws Exception {
		...
		Function<String, StructuredRecord> logTransform = new EventTransform(schema);
	...
	private Schema getSchema(String event) throws Exception {
		...
		if (!jsonElement.isJsonObject())
			throw new Exception(
					String.format("[%s] Things events must be JSON objects.", SseTransform.class.getName()));

		JsonObject eventObject = jsonElement.getAsJsonObject();
		return SseUtil.getSchema(eventObject);
